...
async function get_recent_tweets() {
  ...
  let token = process.env.TWITTER_TOKEN;
  const response = await axios.get(
    twitter_base_url + 'tweets/search/recent' + '?expansions=geo.place_id&tweet.fields=lang,author_id&place.fields=geo,country_code,contained_within&max_results=100',
    {
      params: {
        query: 'Holiday',
      },

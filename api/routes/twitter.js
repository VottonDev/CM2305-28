...
app.get('/get_tweet_geolocation', async (req, res) => {
  ...
  let token = req.body.token;
  const response = await axios.get('https://api.twitter.com/2/tweets/' + tweet_id + '?expansions=geo.place_id&place.fields=contained_within,country,country_code,full_name,geo,id,name,place_type', {
    headers: {
      Authorization: 'Bearer ' + token,
    },
  });

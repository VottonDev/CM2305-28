  ...
  if (response.status === 200) {
    ...
    for (i=0; i<100; i++){
          ...
          if (wc([long,lat])!= null && 'context_annotations' in response.data.data[i]){
            var country_code_hold = wc([long,lat]);
...
async function get_user_details(){
  let token = 'AAAAAAAAAAAAAAAAAAAAAL65WAEAAAAA%2Bz3MLAMpuqpuAbdRVV7l3WUMPxU%3DNez0849RCsrNP6MEKDGGZxYlwxENJA6TBhIOcJTW0bzG2wgHJi';
  const response = await axios.get(
    "https://api.twitter.com/2/users/2244994945/liked_tweets?max_results=5&expansions=referenced_tweets.id&tweet.fields=context_annotations",
    {
      headers: {
        Authorization: 'Bearer ' + token,
      },
